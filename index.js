...
// Aquí colocas toda la historia
var terminalInput = [
  "El material que va a ver a continuación puede ser traumático para algunas personas. Favor tener discreción.",
...
// Colocas dónde deberían ir las preguntas
var decisionInput = [
  "(Agente P) ¿Qué opina doctora?",
  "La cavidad de acceso debe tener una forma:",
  "¿Que N° de fresa debería utilizar al momento de abrir la cámara pulpar del 16?",
...
// Colocas los mensajes de Game Over
var gameOverInput = [
  "Lo hiciste mal",
  "Pero terriblemente mal",
  "Jorribol",
  "@",
];

// Game Over, dependiendo del error
var gameOverAlternatives = [
  ["Lo hiciste mal", "Pero terriblemente mal", "Jorribol", "@"],
  ["No puede abandonarnos en este momento, tiene mucha información.", "@"],
  [
    "(Agente P) Noooo! el paciente no tenía el dispositivo.",
    "El paciente comienza a sangrar de forma exagerada, descubres que tenía la pulpa viva",
    "El paciente estaba anticoagulado",
    "No puedes parar la hemorragia",
    "El paciente muere desangrado",
    "@",
  ],
  [
    "(Agente P) Eres una inútil.",
    "El gobierno no tiene compasión de ti y decide encerrarte en una celda de máxima seguridad por traición.",
    "@",
  ],
  [
    "(Agente P) Haz hecho mal la apertura , por lo tanto no se logran ver los dos conductos radiculares.",
    "(Agente P) Mereces morir. jaja bueno no es para tanto por favor estudia la anatomia y la apertura de premolares",
    "En realidad el Agente P solo estaba intentando ser un buen tipo. Cuando su superior se entera, lo echa de la agencia. Por lo tanto tú que eres su subordinada también quedas sin trabajo.",
    "Ahora el es un indigente y todo es por tu culpa.",
    "@",
  ],
  [
    "Ohh no has extendido la cavidad de forma errónea",
    "Dejaste una zona de la corona muy delgada y se ha fracturado",
    "La intervención se complicó y el paciente no dejó que siguieras trabajando en él.",
    "Tus superiores están muy decepcionados y creen que ya no mereces hacer parte del equipo.",
    "@",
  ],
];
...
var decisiones = [
  ...
  [
    "Realizar una apertura inicial en la fosa central.",
    "Tomar una radiografía periapical del 14",
    "No lo sé... No soy científica.",
  ],
...
function showScreen() {
  ...
  switch (currentStage) {
    ...
    case "terminal":
      if (
        decisionInput.indexOf(terminalInput[currentInput]) != -1 &&
        currentStage != "decision"
      ) {
        if (
          terminalInput[currentInput] != "¿Comó se debe realizar la apertura?"
        ) {
          currentStage = "decision";
        } else {
...
function showIntro() {
  ...
  if (randomTint()) {
    ...
    image(
      img,
      windowWidth / 2 + Math.floor(Math.random() * 30),
      windowHeight / 2 + Math.floor(Math.random() * 30),
      img.width / 5,
      img.height / 5
    );
    tint(0, 255, 255);
    image(
      img,
      windowWidth / 2 + Math.floor(Math.random() * 30 * (Math.random() - 1)),
      windowHeight / 2 + Math.floor(Math.random() * 30 * (Math.random() - 1)),
      img.width / 5,
      img.height / 5
    );
  } else {
...
function showTerminal() {
  ...
  if (randomType() && count == 5) {
    count = 0;
    text(
      storyText() + showPuntero(),
      terminalX + padding + random(padding),
      terminalY + padding + 10,
      windowWidth - 180,
      windowHeight - 120
    );
  } else {
...
function showDecision() {
  ...
  for (i = 0; i < decisiones[currentDecision].length; i++) {
    posX =
      terminalX +
      padding +
      ((windowWidth - 180) / decisiones[currentDecision].length) * i;
  ...
  for (i = 0; i < 3; i++) {
    ...
    if (i == 0) {
      image(
        ...
        posX,
        posY,
        im1.width * (widthX / im1.width) - 10,
        im1.height * (heightY / im1.height) - 10
      );
    } else if (i == 1) {
      image(
        im2,
...
function showGameOver() {
  if (youGood) {
    ...
    if (randomType() && count == 5) {
      ...
      text(
        s,
        windowWidth / 2 + random(100) * (random(2) - 1),
        windowHeight / 3 + random(100) * (random(2) - 1)
      );
      push();
      textSize(50);
      text(
        s2,
        windowWidth / 2 + random(100) * (random(2) - 1),
        windowHeight / 2 + random(100) * (random(2) - 1) + 50,
        windowWidth / 2,
        windowHeight
      );
      pop();
    ...
    image(
      logo,
      windowWidth / 2,
      windowHeight - windowHeight / 6 + 80,
      logo.width / 5,
      logo.height / 5
    );
    push();
    ...
    textAlign(CENTER);
    text(
      "@Carolina Cárdenas, Estudiante de Odontología",
      windowWidth / 2,
...
function mouseClicked() {
  if (currentStage == "decision") {
    ...
    if (correctas[pregunta].indexOf(elegida) != -1) {
      ...
      gameOverInput = currentStage = "gameOver";
    }
